...
def normalize_points(points):
    ...
    norm_points = norm_points_homogeneous[:, :2]  # Convert back to Cartesian coordinates
...
def calculate_fund_matrix(pts1, pts2):
    ...
    for (x1, y1), (x2, y2) in zip(norm_pts1, norm_pts2):
        A.append([x1 * x2, x1 * y2, x1, y1 * x2, y1 * y2, y1, x2, y2, 1]) # 8 point algorithm matrix
    A = np.array(A)
...
def get_epipolar_line(img1, img2, F, pts1, pts2):
    ...
    pts1_hom = np.hstack([pts1, np.ones((pts1.shape[0], 1))]) # Just adding one at the end
    ...
    for r, pt1, pt2 in zip(lines1, pts1, pts2):
        ...
        # Line in img2
        r2 = lines2[np.where((lines1 == r).all(axis=1))[0][0]]  # Corresponding line in img2
